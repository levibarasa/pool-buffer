...
Make sure you have Rust > 1.46.0. Updating the rust toolkit is pretty easy, just do:
...
These crates are:
- `cli-crust` : a command line interface client binary application that can connect and issue commands/queries to a running CrustyDB server.
...
- `optimizer` : an empty crate for an optional milestone to implement query optimization
...
- `txn_manager` : a near empty crate for an optional milestone to implement transactions. the use a `transaciton` is embedded in many other crates, but can be safely ignored for the given milestones.
...
```
[crustydb]>> CREATE TABLE test (a INT, b INT);
```
